...
class JobAnalyzer implements Agent, HasStructuredOutput
{
    ...
    public function schema(JsonSchema $schema): array
    {
        ...
        return [
            ...
            'required_skills' => $schema->array($skillItem)->required(),
            'preferred_skills' => $schema->array($skillItem)->required(),
            'experience_profile' => $schema->object([
                ...
                'project_types' => $schema->array($schema->string()),
            ...
            'language_guidance' => $schema->object([
                'key_terms' => $schema->array($schema->string()),
                'tone' => $schema->string(),
                'phrases_to_mirror' => $schema->array($schema->string()),
            ])->required(),
            'red_flags' => $schema->array($schema->string())->required(),
            'company_research' => $schema->object([
                'industry' => $schema->string(),
                'size_indicators' => $schema->string(),
                'growth_stage' => $schema->string(),
                'notable_details' => $schema->array($schema->string()),
            ]),
        ];
    }
}
