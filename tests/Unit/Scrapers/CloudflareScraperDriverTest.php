...
test('scrape falls through to Phase 2 when Phase 1 returns shell content', function () {
    ...
    // Phase 2 request — broader selectors for SPA content, networkidle0
    expect($requests[1][0]->data()['gotoOptions']['waitUntil'])->toBe('networkidle0');
    expect($requests[1][0]->data()['waitForSelector']['timeout'])->toBe(30000);
    expect($requests[1][0]->data()['waitForSelector']['selector'])->toContain('h1');
    expect($requests[1][0]->data()['waitForSelector']['selector'])->toContain('article');
